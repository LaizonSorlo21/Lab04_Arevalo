...
/* ------------------------------------------------------------
  Normalizar datos: el laboratorio se entrega sin tildes para
  evitar problemas de codificacion al ejecutar el script.
   ------------------------------------------------------------ */
UPDATE dbo.Categorias SET NombreCategoria=CASE CategoriaID WHEN 4 THEN N'Lacteos' ELSE NombreCategoria END, Descripcion=CASE CategoriaID WHEN 1 THEN N'Refrescos, cafes, tes, cervezas y otras bebidas' WHEN 4 THEN N'Quesos y otros productos lacteos' ELSE Descripcion END;
UPDATE dbo.Proveedores SET CompaniaNombre=CASE ProveedorID WHEN 1 THEN N'Lacteos Garcia S.A.' ELSE CompaniaNombre END, NombreContacto=CASE ProveedorID WHEN 1 THEN N'Ana Garcia' WHEN 2 THEN N'Carlos Ramirez' WHEN 3 THEN N'Maria Torres' WHEN 5 THEN N'Lucia Fernandez' ELSE NombreContacto END, CargoContacto=CASE ProveedorID WHEN 3 THEN N'Coordinadora de Distribucion' ELSE CargoContacto END, Direccion=CASE ProveedorID WHEN 1 THEN N'Av. Los Alamos 245' WHEN 5 THEN N'Jr. San Martin 77' ELSE Direccion END, Pais=N'Peru';
UPDATE dbo.Clientes SET NombreContacto=CASE ClienteID WHEN 3 THEN N'Luis Chavez' WHEN 5 THEN N'Miguel Angel Paredes' ELSE NombreContacto END, Empresa=CASE ClienteID WHEN 3 THEN N'Distribuidora Surena EIRL' ELSE Empresa END, Pais=N'Peru';
UPDATE dbo.Empleados SET Apellidos=CASE EmpleadoID WHEN 1 THEN N'Perez Gomez' WHEN 2 THEN N'Lopez Diaz' WHEN 4 THEN N'Vargas Castro' WHEN 5 THEN N'Fernandez Rios' ELSE Apellidos END, Nombre=CASE EmpleadoID WHEN 2 THEN N'Maria' WHEN 4 THEN N'Sofia' ELSE Nombre END, Pais=N'Peru';
UPDATE dbo.Transportistas SET CompaniaNombre=CASE TransportistaID WHEN 1 THEN N'Transportes Rapido SAC' WHEN 2 THEN N'Envios Seguros EIRL' WHEN 3 THEN N'Logistica del Pacifico' ELSE CompaniaNombre END;
UPDATE dbo.Productos SET NombreProducto=CASE ProductoID WHEN 1 THEN N'Cafe Andino Premium' WHEN 2 THEN N'Salsa de Aji Amarillo' ELSE NombreProducto END;
UPDATE dbo.Pedidos SET PaisDestino=N'Peru', Destinatario=CASE PedidoID WHEN 3 THEN N'Distribuidora Surena EIRL' ELSE Destinatario END;
GO
